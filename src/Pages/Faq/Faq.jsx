...
const buttons = ["General", "Courses", "Super 30", "Refer and earn", "Payment"];

function Faq() {
  const [element, setElement] = useState(0);
  return (
    <div className="my-[40px] flex justify-center px-[20px] md:px-[65px] text-[#101010] dark:text-white">
      <div className="max-w-[1000px] flex flex-col gap-10">
        <h3 className="text-center md:text-[36px]  text-[25px] font-medium text-transparent bg-clip-text text-gradient bg-gradient-to-r from-[#4BA25D] via-[#3DA5BD] to-[#905BEC] ">
          Frequently Asked Questions
        </h3>
        <div className="flex justify-center gap-1 border lg:border-2 border-[#37ED81] p-2 rounded-[28px] lg:rounded-full flex-wrap">
          {buttons.map((btn, index) => (
            <button
              type="button"
              onClick={() => setElement(index)}
              key={index}
              className={`duration-200 py-3 px-6 font-medium text-[16px] lg:text-[18px] flex-grow rounded-full hover:bg-[#37ED81] ${
        ...
        <div className="join join-vertical w-full">
          {faqs?.map((faq, i) => (
            <div key={i} className="collapse collapse-arrow join-item">
              ...
              <div className="collapse-title text-[15px] md:text-[18px] font-medium">
                {faq?.q}
              </div>
